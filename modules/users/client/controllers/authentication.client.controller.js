  ...
  function AuthenticationController($scope, $state, UsersService, $location, $window, Authentication, PasswordValidator, Notification) {
    ...
    function onUserSignupSuccess(response) {
      ...
      */
      if(response.roles[0] === 'admin'){
        $state.go($state.previous.state.name || 'admin.articles.list', $state.previous.params);
      }
      else if(response.roles[0] === 'user' ){
        $state.go($state.previous.state.name || 'articles.list', $state.previous.params);
      }
    }
    ...
    function onUserSigninSuccess(response) {
      ...
      */
      ...
      console.log(response.roles[0] == "user");
    }
